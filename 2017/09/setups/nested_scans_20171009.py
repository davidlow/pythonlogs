...
TEMPSup = list(np.linspace(.3,1,25)) + [1.02,1.03,1.04]
#TEMPSup = [.3,.31]
#DWELLTIME = 5 
...
def wait_until_t(obj, targetT, lakeshore, spread=.01, numsamples=20,
                 maxwaitbtwpts = 2):
    starttime = time.time()
    ...
    while (starttime + 1200 > time.time()):
        ...
        print('[Tmean, Tstd, Tmin, Tmax] = ' +
              '[{0:3.3f},{3:3.3f},{1:3.3f},{2:3.3f}]'.format(
...
pre_run_args        = [ [] for t in temperatures]
